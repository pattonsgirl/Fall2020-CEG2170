...
void scan_file(node_t * headp){
  FILE * input = fopen("data.txt", "r");
  node_t * temp = headp;
  //store information in current node
  //use a while loop to go until EOF
  while(fscanf(input, "%d", &temp->num) != EOF){
    //confirm we stored data
    //printf("%d\n", temp->num);
    //create a new node
    node_t * new_node = (node_t *)malloc(sizeof(node_t));
    //store new nodes address in temp's (current node's) next
    temp->next = new_node;
    //move to new_node;
    temp = new_node;
  }
  //now we have the final temp (last node we created)
  //set it as the caboose
  temp->next = NULL;
}
